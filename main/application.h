...
#define MAIN_EVENT_MQTT_CONNECTED (1 << 13)
#define MAIN_EVENT_MQTT_DISCONNECTED (1 << 14)
#define MAIN_EVENT_MQTT_DATA (1 << 15)
...
private:
    ...
    std::string mqtt_received_data_;
    std::string mqtt_received_topic_;
    ...
    void HandleMqttConnectedEvent();
    void HandleMqttDisconnectedEvent();
    void HandleMqttDataEvent();
    ...
    // MQTT methods
    void MqttInit();
    void MqttPublish(const std::string& topic, const std::string& data, int qos = 1,
                     int retain = 0);
    void MqttSubscribe(const std::string& topic, int qos = 1);
